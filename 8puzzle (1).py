...
class Node:
    def __init__(self, data, level, fval):
        ...
        self.data = data #data=start matrix
        self.level = level
        self.fval = fval
...
class Puzzle:
    def __init__(self, size): #size as input
        # Initialize the puzzle size by the the specified size,open and closed lists to empty
        self.n = size #by self.n we have initialized puzzle size 
        self.open = []
        self.closed = []

    def accept(self):
        # Accepts the puzzle from the user
        puz = [] #puzz list is empty
        for i in range(0, self.n):
            temp = input().split(" ") #accepting input 
            puz.append(temp) #storing in puz
        return puz #return the puzzle
    ...
    def process(self):
        ...
        start = self.accept() #calling accept function and storing in start variable
        print("enter the goal state matrix \n")
        goal = self.accept()  #calling accept function and storing in start variable
        start = Node(start, 0, 0) #level and fval are 0
        start.fval = self.f(start, goal) #updated f value
        # put the start node in the open list
        self.open.append(start) #append open list with updated values
        print("\n\n")
        while True:
            cur = self.open[0]
            print("==================================================\n")
...
puz = Puzzle(3) #initialized 3*3 puzzle
puz.process() #puz is calling process function
